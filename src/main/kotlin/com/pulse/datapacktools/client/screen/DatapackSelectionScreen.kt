...
class DatapackSelectionScreen() : Screen(Text.translatable("gui.datapacktools.datapack_selection.screen_title")) {
    ...
    private var availableDatapacks: List<String> = emptyList()
        ...
        repeat(maxDatapacks) { i ->
            val btn = ButtonWidget.builder(Text.literal("")) {
                ...
                if (!name.isNullOrBlank()) {
                    ClientPackets.sendSetDefaultDatapackPacket(name)
                    close()
                }
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        if (availableDatapacks.isEmpty()) {
            context.drawCenteredTextWithShadow(
                textRenderer,
                Text.translatable("gui.datapacktools.datapack_selection.no_datapacks"),
                width / 2,
                height / 2 - 10,
                Colors.GRAY
            )
